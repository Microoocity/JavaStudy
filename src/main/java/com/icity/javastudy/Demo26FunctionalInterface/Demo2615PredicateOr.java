...
/*
    需求：判断一个字符串，有2个判断条件
        1.判断字符串长度是否大于5
        2.判断字符串中是否包含a
    满足1个条件即可，可以使用或运算符连接两个条件

    Predicate接口中有一个方法or，表示或者关系，也可用于连接2个判断条件
    default Predicate<T> and(Predicate<? super T> other) {
        Objects.requireNonNull(other);
        return (t) -> test(t) || other.test(t);
 */
public class Demo2615PredicateOr {
    /*
    定义一个方法，方法的参数传递一个字符串
    传递2个Predicate接口：
        一个用于判断字符串的长度是否大于5
...
//        return predicate1.test(string) || predicate2.test(string);
        return predicate1.or(predicate2).test(string); // 等价于 return predicate1.test(string) || predicate2.test(string);
    }
